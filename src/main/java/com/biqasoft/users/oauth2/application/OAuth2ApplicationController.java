...
public class OAuth2ApplicationController {
    ...
    public Mono<Void> deleteOauthApplicationById(@PathVariable("id") String id){
        return oAuth2ApplicationRepositoryRepository.deleteOauthApplicationById(id).flatMap(x -> Mono.empty());
    }

    @PreAuthorize("isAuthenticated()")
    @ApiOperation(value = "create New Application")
    @RequestMapping(value = "", method = RequestMethod.POST)
    public Mono<OAuth2Application> createNewApplication(@RequestBody OAuth2Application application) {
    ...
    public Mono<OAuth2Application> updateApplication(@RequestBody OAuth2Application application){
    ...
    public Mono<SampleDataResponse> getSecretCodeForOAuthApplication(@PathVariable("id") String id) {
        return oAuth2ApplicationRepositoryRepository.getSecretCodeForOAuthApplication(oAuth2ApplicationRepositoryRepository.findOauthApplicationById(id).block());
    }
